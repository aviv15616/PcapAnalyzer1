...
def plot_tls_distinction(pcap_data):
    ...
    distinction_scores = np.random.uniform(0.1, 1.0, len(services))  # Simulated values
...
def show_graphs_menu(pcap_data):
    ...
    feature_columns = [col for col in pcap_data.columns if
                       col not in ["PcapFile", "TLS Fingerprint", "Cipher Suite", "TLS Version"]]
